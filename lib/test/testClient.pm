...
sub new
{
    ...
    my $self = {
	client => test::testClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a test.FilterContigsParams
$return is a test.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a test.workspace_name
	contigset_id has a value which is a test.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a test.ws_contigset_id
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for test::testClient\n";
    }
    if ($sMajor == 0) {
        warn "test::testClient version is $svr_version. API subject to change.\n";
    }
}
...
package test::testClient::RpcClient;
